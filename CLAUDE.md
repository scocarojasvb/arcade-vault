...
- **`mobile-porter` subagent** (`.claude/agents/mobile-porter.md`) — also writes code directly: picks up the general mobile layout responsiveness that SPEC 10 (touch controls) explicitly left out of scope. Audits the 7 real routes (`/`, `/biblioteca`, `/juego/[id]`, `/juego/[id]/jugar`, `/salon-de-la-fama`, `/acerca-de`, `/auth`) against a fixed 900/720/520px breakpoint ladder and fixes `app/globals.css` (only inside `@media` blocks), `app/layout.tsx`, page components, and only the CSS wrapper/`className` around each game's `<canvas>` — never a game's draw logic or its resolution constants (`W`/`H`/`CELL`/`COLS`/`ROWS`). Verifies with `npm run build` after each route and tracks per-route status in `references/mobile-ready-pages.md`, skipping routes already marked `completo` there unless named explicitly.
...
- Usa el subagente `mobile-porter` (`.claude/agents/mobile-porter.md`) para que las 7 rutas reales de la app se vean bien en navegador móvil — retoma el rediseño responsive que SPEC 10 (controles táctiles) dejó explícitamente fuera de alcance. Como `skin-designer`, escribe código directo (CSS dentro de `@media` en `app/globals.css`, páginas, y solo el escalado CSS del canvas en `app/games/<id>/<id>-game.tsx`, nunca su lógica ni sus constantes de resolución); se le puede indicar una lista concreta de rutas y mantiene una tabla de estado en `references/mobile-ready-pages.md` (nunca reprocesa una ruta ya marcada `completo` salvo que se lo pidas por nombre).
