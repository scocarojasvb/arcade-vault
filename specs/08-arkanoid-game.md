...
- Pausa: **doble control simultáneo**, a diferencia de asteroids/tetris. El botón "PAUSA"/"REANUDAR" del HUD de React controla el mismo estado de pausa interno del componente, **y** el componente además escucha las teclas `P`/`Escape` como atajo propio (igual que el original) para alternar esa misma pausa. Mientras el juego está en pausa (por cualquiera de los dos medios), se dibuja el overlay propio del canvas con los 5 botones clicables de "saltar al nivel" (igual que `drawPauseOverlay`/el listener de `click` del original).
...
- [ ] Tanto el botón "PAUSA"/"REANUDAR" del HUD de React como las teclas `P`/`Escape` alternan el mismo estado de pausa; en pausa se ve el overlay propio con los 5 botones de salto de nivel, y hacer click en uno de ellos salta a ese nivel y reanuda el juego.
...
- **Sí (desvío respecto al patrón de asteroids/tetris):** doble control de pausa — el botón PAUSA del HUD de React y las teclas `P`/`Escape` internas controlan el mismo estado, y se mantiene el overlay propio de selección de nivel por click. A diferencia de asteroids/tetris (donde se descartó la tecla de pausa propia del original para evitar dos UIs), aquí el usuario pidió explícitamente mantener ambas — el overlay de niveles es una funcionalidad de navegación (saltar de nivel) que el HUD de React no ofrece, así que no es una UI de pausa duplicada sino un complemento.
...
| El overlay de selección de nivel por click depende de coordenadas de mouse escaladas (`scaleX`/`scaleY` sobre `getBoundingClientRect`), que puede desalinearse si el canvas se escala de forma no uniforme por CSS.                            | Reutilizar el mismo cálculo de escalado que ya usa el movimiento de la paleta por mouse en el propio `game.js`; verificar manualmente que los 5 botones sean clicables con precisión en distintos tamaños de ventana.                            |
